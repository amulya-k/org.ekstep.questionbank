  ...
  .controller('QuestionFormController', ['$scope', 'pluginInstance', function ($scope, pluginInstance) {
    ...
    $scope.saveCopiedQuestion = function(qData){
      ...
      if(!_.isUndefined(qData.body)){
        ...
        var v1Question = {};
        // v1Question.data.config.metadata.title = "Copy of - " + qData.title;
        var outRelations = [];
        _.each(qData.concepts, function(concept){
          outRelations.push({
            "endNodeId": concept.identifier,
            "relationType": "associatedTo"
          });
        ...
        var metadata = {
          "code": "NA",
          "name": "Copy of - " + qData.name,
          "title": "Copy of - " + qData.title,
          "medium": qData.language,
          "max_score": qData.max_score,
          "gradeLevel": qData.gradeLevel,
          ...
          "createdBy": window.context.user.id,
          "channel": ecEditor.getContext('channel'),
          "type": qData.type, // backward compatibility
          "template": "NA", // backward compatibility
          "template_id": "NA", // backward compatibility
          "framework": ecEditor.getContext('framework')
        };
